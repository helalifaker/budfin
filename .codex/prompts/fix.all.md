...
name: fix:all
description: >
    Run all BudFin fixers in sequence: lint -> types -> tests -> security -> precision. Accepts an
...
- If a quoted string is provided (e.g., `"enrollment POST returns 500"`): treat as a **symptom** for root cause analysis
- If `--lint`, `--types`, `--tests`, `--security`, or `--precision` flag is present: run ONLY that single fixer (escape hatch)
- If no argument: run all fixers in sequence

## Escape-Hatch Mode (single flag)

If a `--[fixer]` flag was passed, run ONLY that fixer and its final verification:

| Flag          | Fixer                     | Verification                      |
| ------------- | ------------------------- | --------------------------------- |
| `--lint`      | `/fix:lint` workflow      | `pnpm lint` returns 0             |
| `--types`     | `/fix:types` workflow     | `pnpm typecheck` returns 0        |
| `--tests`     | `/fix:tests` workflow     | `pnpm test` passes                |
| `--security`  | `/fix:security` workflow  | security-reviewer PASS            |
| `--precision` | `/fix:precision` workflow | financial-precision-reviewer PASS |

After the single fixer completes, run final verification and output a one-line summary. Stop.

## Full Mode (no flag or symptom only)

### Step 0 -- Debug (only if symptom provided)

If a symptom argument was provided (e.g., `"enrollment POST returns 500"`):
