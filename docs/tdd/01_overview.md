...
**Infrastructure Layer.** Docker Compose orchestrates four containers: `nginx` (reverse proxy, TLS termination, static asset serving), `api` (Node.js application), `db` (PostgreSQL 16), and `worker` (background job processing for exports and large imports). GitHub Actions runs CI/CD: lint, type-check, unit tests, integration tests, and Docker image builds. PostgreSQL backups run on a cron schedule via `pg_dump`. UptimeRobot or equivalent monitors the `/api/v1/health` endpoint.
