...
```json
{
  ...
  "field_errors": [
    { "field": "headcount", "message": "Must be >= 0" }
  ]
}
```
...
```json
{
  ...
  "annual_totals": {
    "primary": { "total_revenue_ht": "string", "total_staff_costs": "string", "net_profit": "string" },
    "comparison": { "total_revenue_ht": "string", "total_staff_costs": "string", "net_profit": "string" },
    "variance_absolute": { "total_revenue_ht": "string", "total_staff_costs": "string", "net_profit": "string" },
    "variance_pct": { "total_revenue_ht": "string|null", "total_staff_costs": "string|null", "net_profit": "string|null" }
  }
}
```
...
```json
{
  ...
  "preview": [
    { "grade_level": "string", "student_count": "number" }
  ]
}
```
...
```json
{
  ...
  "summary": {
    ...
    "over_capacity_grades": [
      { "grade_level": "string", "utilization_pct": "string" }
    ]
  }
}
```
...
export const CreateVersionSchema = z.object({
  ...
  type: z.enum(['Budget', 'Forecast'])
    .openapi({ description: 'Version type' }),
  description: z.string().max(500).optional()
    .openapi({ description: 'Optional description' }),
  source_version_id: z.number().int().positive().optional()
...
```typescript
export const MonetaryStringSchema = z.string()
