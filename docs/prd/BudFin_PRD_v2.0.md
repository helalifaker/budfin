...
| `base_salary` | `DECIMAL(15,4)` | `NOT NULL, CHECK (>= 0)` | Core monthly salary in SAR. |
| `housing_allowance` | `DECIMAL(15,4)` | `NOT NULL, DEFAULT 0, CHECK (>= 0)` | Monthly housing allowance (IL) in SAR. Included in EoS base. |
| `transport_allowance` | `DECIMAL(15,4)` | `NOT NULL, DEFAULT 0, CHECK (>= 0)` | Monthly transport allowance (IT) in SAR. Included in EoS base. |
| `responsibility_premium` | `DECIMAL(15,4)` | `NOT NULL, DEFAULT 0, CHECK (>= 0)` | Monthly responsibility premium in SAR. Included in EoS base. |
| `hsa_amount` | `DECIMAL(15,4)` | `NOT NULL, DEFAULT 0, CHECK (>= 0)` | Monthly HSA amount in SAR. Paid on 10-month basis; excluded during Jul-Aug. NOT included in EoS base. |
...
| `version_id` | `INT` | `FOREIGN KEY -> budget_versions.version_id, NOT NULL` | Budget version this fee grid belongs to (FR-REV-004). |
...
| `last_modified_at` | `TIMESTAMP` | `NOT NULL, DEFAULT CURRENT_TIMESTAMP ON UPDATE` | Last modification timestamp. |
| `locked_at` | `TIMESTAMP` | `NULL` | Timestamp when locked. NULL if not yet locked. |
...
| `source_version_id` | `INT` | `FOREIGN KEY -> budget_versions.version_id, NULL` | If cloned from another version (FR-VER-005). NULL for originals. |
...
| `version_id` | `INT` | `FOREIGN KEY -> budget_versions.version_id, NOT NULL` | Budget version. |
...
| `timestamp` | `TIMESTAMP` | `NOT NULL, DEFAULT CURRENT_TIMESTAMP` | When the change occurred, stored in UTC. Displayed in AST. |
...
| `version_id` | `INT` | `FOREIGN KEY, NULL` | Budget version context. NULL for system-level changes. |
