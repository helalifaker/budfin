...
- [ ] AC-07: Given a user with 2 active sessions (the configured max_sessions_per_user), when a 3rd login occurs, then the oldest session's refresh token is revoked, a new session is created, and the advisory lock, session eviction, token-family creation, last-login update, and login audit all complete in the same database transaction.
...
- [ ] AC-16: Given a refresh token that belongs to an inactive user, when POST /api/v1/auth/refresh is called, then the response is 401 with code ACCOUNT_DISABLED, the refresh_token cookie is cleared, any remaining active refresh tokens for that user are revoked, and an audit entry with operation REFRESH_REJECTED_INACTIVE is created.
- [ ] AC-17: Given an Admin deactivates a user via PATCH /api/v1/users/:id with `{ "is_active": false }`, then the user is marked inactive and all active refresh tokens for that user are revoked in the same transaction as the update.
...
| POST   | /api/v1/auth/logout   | No body (reads refresh_token cookie)                                                   | 204 No Content + clear cookie                                                                        | None (public)   |
| GET    | /api/v1/context       | --                                                                                     | `{ user: { id, email, role }, schoolYear: string \| null, permissions: string[] }`                   | Authenticated   |
...
| PATCH  | /api/v1/users/:id     | `{ role?, is_active?, force_password_reset?, unlock_account?, force_session_revoke? }` | Updated user object; deactivation automatically revokes active refresh-token sessions                | Admin           |
...
| EC-AUTH-02 | Concurrent login race condition (two logins at exact same time for same user at session limit) | Use a database advisory lock on user_id and keep session counting, eviction, token creation, and audit logging inside the same transaction to serialize concurrent login attempts.                                   |
...
| EC-AUTH-07 | Inactive user still holds a refresh token cookie                                               | Refresh requests for inactive users are rejected with ACCOUNT_DISABLED, the refresh cookie is cleared, the user's active refresh tokens are revoked, and the event is logged as REFRESH_REJECTED_INACTIVE.           |
