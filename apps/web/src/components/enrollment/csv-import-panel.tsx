			...
			<SheetContent side="right" className="w-120 overflow-y-auto sm:max-w-120">
				...
				<div className="mt-6 space-y-4">
					...
					<div>
						<label
							htmlFor="import-year"
							className="mb-1 block text-(--text-sm) font-medium text-(--text-primary)"
						>
							Academic Year
						</label>
					...
					<div>
						...
						<input
							...
							className="block w-full text-(--text-sm) text-(--text-muted) file:mr-4 file:rounded-(--radius-sm) file:border-0 file:bg-(--workspace-bg-muted) file:px-4 file:py-2 file:text-(--text-sm) file:font-medium file:text-(--text-primary) hover:file:bg-(--workspace-bg-muted)"
						/>
						{file && (
						...
						{validationResult && validationResult.validRows > 0 && (
							<>
								{validationResult.errors.length > 0 && (
									<p className="text-(--text-xs) text-(--color-warning)">
					...
					{validationResult && (
						<div className="rounded-(--radius-lg) border p-4 space-y-3">
							...
							{/* Error list */}
							{validationResult.errors.length > 0 && (
								<div className="max-h-32 overflow-y-auto text-(--text-xs) text-(--color-error)">
					...
					{/* Success */}
					{importSuccess && (
						<div className="flex items-center gap-2 rounded-(--radius-lg) bg-(--color-success-bg) px-4 py-3 text-(--text-sm) text-(--color-success)">
