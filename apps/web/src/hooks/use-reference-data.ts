...
export function useNationalities() {
	return useQuery({
		...
		queryFn: () => apiClient<{ nationalities: Nationality[] }>('/master-data/nationalities'),
...
export function useTariffs() {
	return useQuery({
		queryKey: ['tariffs'],
		queryFn: () => apiClient<{ tariffs: Tariff[] }>('/master-data/tariffs'),
...
export function useCreateTariff() {
	const queryClient = useQueryClient();
	return useMutation({
		mutationFn: (body: { code: string; label: string; description?: string | undefined }) =>
...
export type BandMapping = 'MATERNELLE' | 'ELEMENTAIRE' | 'COLLEGE' | 'LYCEE' | 'NON_ACADEMIC';
...
export function useDepartments() {
	return useQuery({
		...
		queryFn: () => apiClient<{ departments: Department[] }>('/master-data/departments'),
...
export function useCreateDepartment() {
	const queryClient = useQueryClient();
	return useMutation({
		mutationFn: (body: { code: string; label: string; bandMapping: BandMapping }) =>
