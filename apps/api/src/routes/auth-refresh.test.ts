...
vi.mock('../services/token-family.js', () => ({
	...
	rotateToken: (...args: unknown[]) =>
		mockRotateToken(...args),
	detectReplay: (...args: unknown[]) =>
		mockDetectReplay(...args),
}))
...
describe('POST /api/v1/auth/refresh', () => {
	it('returns new access_token + Set-Cookie on valid refresh',
		async () => {
	...
	it('returns 401 REFRESH_TOKEN_REUSE for revoked token',
		async () => {
			const app = await buildTestApp()
			mockRefreshTokenFindFirst.mockResolvedValue({
				id: 10,
			...
			mockDetectReplay.mockResolvedValue(
			...
			expect(mockDetectReplay).toHaveBeenCalledWith(
				expect.any(String),
				1,
				expect.any(String),
			)
...
describe('POST /api/v1/auth/logout', () => {
	...
	it('creates LOGOUT audit entry', async () => {
		...
		const data = (
			logoutAudit![0] as { data: { userId: number } }
		).data
