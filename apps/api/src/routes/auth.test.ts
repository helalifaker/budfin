...
describe('POST /api/v1/auth/login', () => {
	it('returns 200 + access_token + Set-Cookie for valid creds', async () => {
	...
	it('triggers lockout after threshold failures and returns ACCOUNT_LOCKED', async () => {
		...
		const lockAudit = mockAuditEntryCreate.mock.calls.find(
			(c: unknown[]) =>
				(c[0] as { data: { operation: string } }).data.operation === 'ACCOUNT_LOCKED'
		);
	...
	it('enforces session concurrency — revokes oldest on max', async () => {
		const app = await buildTestApp();
		mockUserFindUnique.mockResolvedValue({ ...baseUser });
		mockVerifyPassword.mockResolvedValue(true);
		...
		const revokeAudit = mockAuditEntryCreate.mock.calls.find(
			(c: unknown[]) =>
				(c[0] as { data: { operation: string } }).data.operation === 'SESSION_FORCE_REVOKED'
		);
	...
	it('creates LOGIN_SUCCESS audit entry', async () => {
		...
		const audit = mockAuditEntryCreate.mock.calls.find(
			(c: unknown[]) => (c[0] as { data: { operation: string } }).data.operation === 'LOGIN_SUCCESS'
		);
