...
function formatEmployee(raw: DecryptedEmployee, redactSalary: boolean) {
	return {
		...
		name: raw.name,
		functionRole: raw.function_role,
		department: raw.department,
		status: raw.status,
		joiningDate: raw.joining_date,
		paymentMethod: raw.payment_method,
		isSaudi: raw.is_saudi,
		isAjeer: raw.is_ajeer,
		isTeaching: raw.is_teaching,
		hourlyPercentage: raw.hourly_percentage,
		baseSalary: redactSalary ? null : raw.base_salary,
		housingAllowance: redactSalary ? null : raw.housing_allowance,
		transportAllowance: redactSalary ? null : raw.transport_allowance,
		responsibilityPremium: redactSalary ? null : raw.responsibility_premium,
		...
		serviceProfileId: raw.service_profile_id,
		homeBand: raw.home_band,
		contractEndDate: raw.contract_end_date,
		disciplineName: raw.discipline_name,
		serviceProfileName: raw.service_profile_name,
...
export async function employeeRoutes(app: FastifyInstance) {
	...
	app.get('/employees', {
		...
		handler: async (request, reply) => {
			...
			return {
				data: employees.map((e) => formatEmployee(e, redactSalary)),
				total,
				page,
				page_size,
			};
