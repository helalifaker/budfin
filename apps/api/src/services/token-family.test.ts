...
import { createFamily, rotateToken, detectReplay, revokeAllUserTokens } from './token-family.js';
...
describe('token-family service', () => {
	describe('createFamily', () => {
		...
		it('creates RefreshToken record with correct expiresAt', async () => {
			...
			expect(result.expiresAt.getTime()).toBeGreaterThanOrEqual(before + eightHoursMs);
			expect(result.expiresAt.getTime()).toBeLessThanOrEqual(after + eightHoursMs);
